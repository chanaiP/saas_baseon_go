...
- [ ] 实现工作台 API：基于品牌、商品、SKU、Skill、热点和用户提示生成母稿。（已完成可持久化的基础生成，待接 AI 能力中心）
- [ ] 接入 AI 能力中心场景：母稿生成、渠道改写、审核建议，写入 AI 调用审计与失败信息。
...
- [ ] 所有写接口补后端权限校验、套餐功能校验、配额校验/扣减和审计日志。（已完成路由权限和审计，待补套餐/配额运行时校验）
...
- [ ] 为 `ai_geo_monthly_draft_generations`、`ai_geo_monthly_publish_tasks`、品牌数、商品数、渠道账号数实现运行时校验。
...
- [ ] 补 service 单测：资料校验、状态机、配额拒绝、AI 失败降级、发布计划流转。（已完成租户隔离、母稿状态机、跨租户拒绝、审计上下文）
- [ ] 补 handler/API 测试：统一响应、分页、参数错误、权限拒绝、审计触发。
- [ ] 补前端构建和关键页面浏览器复测记录。
- [ ] 更新 `docs/apps/ai-geo.md` 为生产级交付文档，记录接口、状态机、数据模型、风险与上线步骤。
- [ ] 运行相关 Go 测试、前端构建、`git diff --check` 并提交。（本阶段 Go 测试、前端构建、diff 检查已通过，待提交）
