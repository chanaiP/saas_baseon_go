...
- [ ] 补 service 单测：资料校验、状态机、配额拒绝、AI 失败降级、发布计划流转。（已完成租户隔离、资料详情跨租户拒绝、母稿状态机/详情/归档、渠道内容编辑/审核/跨租户拒绝、审计上下文、配额拒绝与月度扣减、AI Gateway 母稿/渠道/审核建议解析、资料导入预校验/部分成功/幂等落库、素材/热点租户隔离、工作台 SKU/热点上下文）
