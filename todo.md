...
- [ ] 实现资料中心 API：品牌、商品、SKU、竞品、关键词/素材的列表、详情、新增、更新、归档。（已完成品牌/商品/SKU/竞品基础列表、详情、新增、更新、归档；待补关键词/素材）
- [x] 实现导入 API：字段映射、预校验、批次记录、错误行记录、幂等导入和部分成功策略。
- [x] 实现总览 API：资料完整度、今日生产进度、待处理事项、渠道接入状态和配额使用。
- [ ] 实现工作台 API：基于品牌、商品、SKU、Skill、热点和用户提示生成母稿。（已完成品牌、商品、Skill、用户提示和可持久化生成，待补 SKU/热点上下文）
...
- [ ] 校验 Manifest 菜单、操作、API、套餐功能点和配额与后端路由完全一致。
...
- [ ] 补 service 单测：资料校验、状态机、配额拒绝、AI 失败降级、发布计划流转。（已完成租户隔离、资料详情跨租户拒绝、母稿状态机/详情/归档、跨租户拒绝、审计上下文、配额拒绝与月度扣减、AI Gateway 母稿/渠道/审核建议解析、资料导入预校验/部分成功/幂等落库）
- [ ] 补 handler/API 测试：统一响应、分页、参数错误、权限拒绝、审计触发。
- [ ] 补前端构建和关键页面浏览器复测记录。（本阶段前端构建已通过，待浏览器复测记录）
- [ ] 更新 `docs/apps/ai-geo.md` 为生产级交付文档，记录接口、状态机、数据模型、风险与上线步骤。（已补接口、状态机、数据模型、AI Gateway、配额运行时门禁、迁移验证与回滚，待最终风险和上线步骤）
- [ ] 运行相关 Go 测试、前端构建、`git diff --check` 并提交。（本阶段 Go 测试已通过，待 diff 检查和提交）
