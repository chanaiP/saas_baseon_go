...
- [ ] 明确二期边界：真实渠道发布、OAuth 授权、Webhook 回调、Agent 发布执行、素材文件治理和跨渠道数据回流。
...
- [ ] 实现资料中心 API：品牌、商品、SKU、竞品、关键词/素材的列表、详情、新增、更新、归档。（已完成品牌/商品/SKU/竞品基础列表与新增、品牌更新）
- [ ] 实现导入 API：字段映射、预校验、批次记录、错误行记录、幂等导入和部分成功策略。（已完成基础预校验、批次记录、错误行记录和部分成功状态，待补字段映射落库和幂等导入）
- [x] 实现总览 API：资料完整度、今日生产进度、待处理事项、渠道接入状态和配额使用。
- [ ] 实现工作台 API：基于品牌、商品、SKU、Skill、热点和用户提示生成母稿。（已完成品牌、商品、Skill、用户提示和可持久化生成，待补 SKU/热点上下文）
- [ ] 接入 AI 能力中心场景：母稿生成、渠道改写、审核建议，写入 AI 调用审计与失败信息。（已接母稿生成 `ai_geo_draft_generation` 和渠道改写 `ai_geo_channel_rewrite`，待补审核建议与失败记录）
- [ ] 实现母稿 API：列表、详情、保存、提交审核、审核通过/驳回、归档。（已完成列表、创建、提交、通过/驳回）
- [ ] 实现渠道内容 API：按渠道生成版本、编辑、审核、状态流转。（已完成 AI Gateway 渠道改写生成）
...
- [ ] 补 service 单测：资料校验、状态机、配额拒绝、AI 失败降级、发布计划流转。（已完成租户隔离、母稿状态机、跨租户拒绝、审计上下文、配额拒绝与月度扣减、AI Gateway 母稿生成解析）
- [ ] 补 handler/API 测试：统一响应、分页、参数错误、权限拒绝、审计触发。
- [ ] 补前端构建和关键页面浏览器复测记录。
- [ ] 更新 `docs/apps/ai-geo.md` 为生产级交付文档，记录接口、状态机、数据模型、风险与上线步骤。（已补母稿 AI Gateway 接入和配额运行时门禁，待最终上线步骤）
