...
- [x] 实现资料中心 API：品牌、商品、SKU、竞品、关键词/素材的列表、详情、新增、更新、归档。
...
- [x] 实现渠道内容 API：按渠道生成版本、编辑、审核、状态流转。
- [x] 实现渠道资料 API：渠道类型、入口、支持能力、默认发布方式、启停。
- [x] 实现渠道账号 API：账号资料、授权状态、可发布状态、失效处理。
- [x] 实现发布计划 API：日历、队列、创建、调整时间、标记已发布、失败重试、发布链接回填。（已完成列表/队列、创建、调整时间、标记已发布、失败重试、发布链接回填、状态机校验、渠道内容状态同步和日历聚合接口）
...
- [x] 为总览、资料中心、工作台、母稿、发布计划、渠道管理补 loading、empty、error 和提交中状态。（已补页面加载/提交中状态、错误 toast、空数据兜底和后端错误透传）
- [x] 写操作接入真实接口并处理后端校验错误、权限错误、套餐/配额错误。
- [x] 按后端权限控制按钮显隐，同时保留后端二次校验。
- [x] 深浅色、移动端、长文本、表格横向滚动和弹窗抽屉边界复测。（已通过前端构建、CSS/布局静态检查和关键交互状态复核）

### 17.5 权限、套餐、配额与租户闭环

- [x] 校验 Manifest 菜单、操作、API、套餐功能点和配额与后端路由完全一致。（已补全量差异脚本 `scripts/verify-ai-geo-manifest-parity.mjs` 并通过核对）
- [x] 为 `ai_geo_monthly_draft_generations`、`ai_geo_monthly_publish_tasks`、品牌数、商品数、渠道账号数实现运行时校验。
- [x] 确认套餐变更、租户菜单覆盖、角色授权变更后即时影响 AI GEO 页面和 API。（已补权限策略测试覆盖套餐/租户覆盖/角色变更对 AI GEO 操作 API 的即时影响）
- [x] 补普通租户、无套餐租户、无角色权限用户、平台管理员的访问差异测试。

### 17.6 测试与交付

- [x] 补 repository tenant scope 测试，证明无法跨租户读写 AI GEO 数据。
- [x] 补 service 单测：资料校验、状态机、配额拒绝、AI 失败降级、发布计划流转。（已完成租户隔离、资料详情跨租户拒绝、母稿状态机/详情/归档、关键词详情/更新/归档/租户隔离、渠道内容编辑/审核/跨租户拒绝、审计上下文、配额拒绝与月度扣减、AI Gateway 母稿/渠道/审核建议解析、资料导入预校验/部分成功/幂等落库、素材/热点详情/更新/租户隔离、发布计划调整/失败重试/发布状态同步、发布日历聚合、工作台 SKU/热点上下文）
- [x] 补 handler/API 测试：统一响应、分页、参数错误、权限拒绝、审计触发。
- [x] 补前端构建和关键页面浏览器复测记录。
