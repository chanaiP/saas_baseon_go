...
用户想换品牌、商品、内容类型、热点、参考写作风格或方向。

行为：
- 更新上下文。
- 重新计算推荐方向。
- 不要直接沿用旧方向生成。
- 如果用户说“换风格、参考这篇写法、按这个语气写、提炼风格、套用这个风格”，优先识别为参考写作风格上下文变化，不要误判为普通生成。
- 如果用户提供 URL 并要求提炼风格，目标对象是 style_template；如果用户提供 URL 并要求提炼热点，目标对象是 hotspot。
