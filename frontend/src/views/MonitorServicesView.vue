...
async function load() {
  if (!pageAction('monservices:view')) return
  loading.value = true
  err.value = ''
  try {
  ...
  <div class="page">
    <el-card v-loading="loading" shadow="never" class="shell-card">
      <template #header>
        <div class="hdr">
          <span>服务监控</span>
          <el-button v-if="pageAction('monservices:view')" type="primary" plain @click="load">刷新</el-button>
        </div>
      </template>
      <el-empty v-if="!pageAction('monservices:view')" description="无查看权限" />
      <template v-else>
