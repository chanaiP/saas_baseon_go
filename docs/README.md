...
4. [C 端个人空间与账号体系](tech_design/C端个人空间与账号体系.md)：了解 Account/UserIdentity/AppUser 分离、个人空间、菜单范围和个人/企业复用能力。
5. [应用中心接入强制规则](tech_design/应用中心接入强制规则.md)：新增应用前必须阅读，确认 `app_code`、Manifest、独立目录、装载同步和拒绝装载门禁。
6. [独立部署应用接入约束](tech_design/独立部署应用接入约束.md)：`deployment_mode=STANDALONE` 应用必须阅读，确认外部鉴权、通讯模式、Webhook、数据同步、套餐外订阅和 AI agent 门禁。
7. [应用中心](tech_design/应用中心.md)：了解应用中心当前页面、数据模型、统计口径和菜单权限边界。
8. [业务开发标准](tech_design/业务开发标准.md)：新增业务模块前必须阅读，确认租户、权限、套餐、配额、审计和测试准入要求。
9. [模块接入准入清单](tech_design/模块接入准入清单.md)：新增或修改业务能力时逐项检查。
10. [最终交接报告](FINAL_PARITY_REPORT.md)：查看当前 Go 重建版与原项目的最终对齐状态。
...
- C 端个人空间、Account/UserIdentity/AppUser 分离、平台/企业/个人菜单范围控制。
...
新增 C 端个人空间、账号体系和菜单空间范围时，优先阅读 [C 端个人空间与账号体系](tech_design/C端个人空间与账号体系.md)。
...
C 端账号与个人空间相关结构见 [C 端个人空间与账号体系数据库设计](sql_design/C端个人空间与账号体系数据库设计.md)。
...
C 端个人空间验收见 [13-C 端个人空间测试用例](test_cases/13-C端个人空间-测试用例.md)。
...
- 改 `tenant_type`、`tenant_scope`、Account/UserIdentity/AppUser 关系或个人空间能力时，同步更新 C 端个人空间技术方案、数据库设计、测试用例和后续 TODO。
