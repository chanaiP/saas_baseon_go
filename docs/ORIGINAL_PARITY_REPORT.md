...
- 主体与品牌相关覆盖包含主体范围选择、主体/套餐删除保护、品牌字段 trim 和空串清除策略。
- 套餐相关覆盖包含功能到配额映射：用户、组织、导入、导出、API Key、Webhook 等默认套餐能力配额关系。
- 权限相关覆盖包含数据权限必须带 `data_scope`、`CUSTOM` 范围目标校验、数据权限模式合法性和“仅菜单权限可配置数据权限类型”约束。
- 角色相关覆盖已有 application 层测试：创建时权限 ID 去重、空名称拒绝、列表分页默认值、更新时替换权限集合。
- 组织相关覆盖包含组织节点被用户引用时禁止删除、组织节点被业务单元映射引用时禁止删除，以及组织菜单数据范围过滤策略。
- 岗位相关覆盖包含岗位类型被岗位引用时禁止删除、岗位被用户岗位关联引用时禁止删除。
- 业务单元相关覆盖包含业务单元映射组织节点类型规整、组织映射引用保护、数据权限范围引用保护，以及业务单元数据范围过滤策略。
- 字典与参数相关覆盖包含批量参数 key trim/去空/去重、字典项租户覆盖输出、字典/参数引用保护，以及系统参数创建失败时隐藏数据库唯一约束细节。
- 日志相关覆盖包含登录日志/审计日志输出映射、主体名透出、日期过滤参数解析，以及必备审计事件清单。
- 文件与批量相关覆盖包含 file_id 十六进制校验、原始文件名和扩展名安全处理、CSV 公式注入防护、UTF-8 BOM 处理、导入错误截断，以及租户容量统计跳过回收目录。
- 监控、限流与安全守卫相关覆盖包含 Redis info 解析、cache key limit/pattern 规范化和非法 pattern 拒绝、内置 Redis 定时任务清单、登录 IP 限流 key、会话 key、订阅登录保护和权限/菜单到套餐功能映射。
...
- O1-O13 认证、用户、主体/品牌、套餐、权限、角色、组织、岗位、业务单元、数据范围、字典参数、日志、文件批量、监控限流与安全守卫测试等价已完成。
