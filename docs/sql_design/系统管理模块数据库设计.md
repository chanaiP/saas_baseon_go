...
数据库：PostgreSQL 16，字符集 `utf8mb4`。Docker 配置位于 `docker-compose.yml`：
...
- 当前 ORM 未定义 `tenant_id + code` 唯一约束，编码唯一性如需强约束应补迁移【待确认】。
...
当前 ORM 未见 `(dict_type_id, value)` 唯一约束【待确认】。
...
| `tenant_subscription` | `tenant_id`、`plan_id`、`subscription_status`、`start_time`、`end_time` | 【待确认】 | 租户订阅 |
...
风险：`tenant_subscription` 当前 ORM 未见 `(tenant_id)` 当前订阅唯一约束，当前有效订阅唯一性依赖 service 逻辑【待确认】。
...
- 被历史记录引用的数据不得物理删除。
- 唯一字段应墓碑化释放，具体覆盖情况需结合各 Repository 逐项确认【待确认】。
