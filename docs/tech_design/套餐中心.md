...
套餐删除调用 `deletePlan(id)`，前端使用统一归档确认；后端为软删除。功能点和配额项当前未发现独立删除接口【待确认】。未发现批量删除。
...
| 租户订阅 | GET/PUT | `/api/tenants/{tenant_id}/subscription` | 查询/保存订阅 | GET【待确认】、PUT `plan:config` |
...
| 配额使用 | GET | `/api/tenants/{tenant_id}/quota-usage` | 查询使用量 | 【待确认】 |
| 功能校验 | GET | `/api/tenants/{tenant_id}/feature-access/{feature_code}` | 运行时校验 | 【待确认】 |
| 配额校验 | GET | `/api/tenants/{tenant_id}/quota-check/{quota_code}` | 运行时校验 | 【待确认】 |
...
DTO 做类型和必填校验；service 处理套餐编码、功能编码、配额编码唯一性、套餐存在、功能存在、配额存在、配额值合法性、启用功能关联配额过滤、已订阅套餐删除限制【待确认】。
...
- 风险：部分 GET 租户订阅/用量接口权限标识需进一步核对【待确认】。
