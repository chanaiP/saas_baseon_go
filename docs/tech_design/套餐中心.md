...
后续套餐中心功能点和配额的标准来源统一为应用 Manifest 的 `package_features` 与 `quotas`。内置应用也必须提供 Manifest；原先“从菜单/操作自动生成套餐功能点”的逻辑只作为历史兼容和迁移兜底，不再作为新增能力的标准入口。
...
- Manifest 装载服务：读取 `package_features` 与 `quotas` 后同步 `saas_feature`、`saas_quota` 及默认套餐绑定。
- `sync_feature_from_permission()`：历史兼容兜底，用于旧菜单数据补齐功能点；新增应用不得依赖该逻辑作为套餐功能来源。
