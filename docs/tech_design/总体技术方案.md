...
当前工程目录按“前端、后端、文档、数据库初始化”分层。后续新增能力应优先落在既有目录中，避免新建平行结构造成维护分散。
...
├── cmd/api/                          # Gin API 入口
├── cmd/migrate/                      # PostgreSQL schema baseline 与增量迁移入口
├── internal/
...
- Go DTO 与 Gin binding/validator 负责请求和响应契约。
- router 不直接写复杂业务逻辑，调用 service 返回 DTO 或领域结果。

## 15. 异常处理

异常处理由 handler、service 和 `internal/interfaces/http/response` 协作完成：

- 业务错误：转换为统一响应体，保留业务错误码和可读 message。
- 请求参数错误：返回参数错误码和可读提示。
- 未处理异常：服务端记录必要排查信息，客户端返回通用服务器错误，避免泄露 SQL、堆栈和连接信息。
