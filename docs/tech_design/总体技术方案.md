...
可信度说明：本文档基于当前仓库实际实现反向整理。已在代码中确认的内容按确定性描述；无法从代码或脚本中完全确认的内容统一标记为【待确认】。当前项目已提供 `internal/infrastructure/persistence/postgres/migrations/` 作为生产/开发数据库初始化 SQL 入口，但未发现 golang-migrate 版本化迁移目录。
...
| 认证 | Redis session 优先，JWT 降级【具体降级细节以认证服务为准】 | `internal/application/identity`、`internal/infrastructure/security/jwt`、`internal/infrastructure/cache/redis/session` |
| 权限 | 菜单路径、操作码、角色权限、套餐功能 | `internal/interfaces/http/middleware`、`internal/domain/permission` |
...
├── AGENTS.md                         # AI coding agent 项目导航入口
├── CLAUDE.md                         # 工程规则、安全规则、租户隔离、权限与发布验证规则
├── docker-compose.yml                # PostgreSQL、Redis、Gin API、Web 四服务编排
├── .env.example                      # 本地与 Docker 环境变量模板
├── scripts/                          # 本地开发、服务启动、服务检查脚本
│
├── backend/                          # Gin 后端工程
│   ├── Dockerfile                    # Gin API 容器构建文件
│   ├── go.mod              # Python 依赖
│   ├── app/
│   │   ├── main.go                   # Gin 应用入口、路由挂载、启动初始化
│   │   ├── config.go                 # 环境变量与运行配置
│   │   ├── database.go               # GORM DB
│   │   ├── models                 # GORM 数据模型
│   │   ├── api/
│   │   │   ├── middleware               # 鉴权、权限、数据库依赖、健康检查辅助
│   │   │   └── handlers/              # HTTP 路由入口，保持薄 router
│   │   ├── application/                 # 业务编排、权限/套餐/租户上下文、审计等 service
│   │   ├── repositories/                     # 数据访问和可复用持久化操作
│   │   ├── dto/                  # Go DTO 请求、响应、分页契约
│   │   ├── constants/                # 业务常量
│   │   ├── permission       # 菜单 path、操作码、菜单包定义
│   │   └── data_scope.go             # 数据权限范围相关能力
│   ├── scripts/
│   │   └── sql/
│   │       ├── README.md             # 数据库初始化与 Docker 数据库入口说明
│   │       └── prod/                 # 生产/开发双用途 SQL 初始化脚本
│   └── tests/                        # 后端单元测试与安全回归测试
│
...
`docs/` 目录只保留当前有效文档，不再保留历史聊天、阶段报告或临时设计稿。数据库初始化脚本不放在 `docs/`，而是放在 `internal/infrastructure/persistence/postgres/migrations/`，方便开发者、AI 工具和部署流程直接按脚本建库。
