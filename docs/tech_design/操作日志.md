...
分页参数 `skip`、`limit`；筛选参数包括账号/关键字、IP、主体名称、日期范围。日期格式和 limit 上限由 router 或 DTO 控制（当前实现未覆盖，按后续增强处理）。
...
操作日志页面本身只读，不再记录查询行为（当前实现未覆盖，按后续增强处理）。业务日志写入由其他 service 调用。

| 操作 | 是否记录 | 记录内容 | 实现位置 |
|---|---|---|---|
| 查询操作日志 | 后续增强 | 查询条件 | （当前实现未覆盖，按后续增强处理） |
...
- 异常处理：非平台用户传主体筛选应被忽略或拒绝（当前实现未覆盖，按后续增强处理）。
- 注意事项：不能仅依赖前端隐藏主体筛选。
