...
|---|---|---|---|---|
| 批量读取参数 | GET | `/api/sys-params/batch` | 按 key 读取当前值 | 登录（当前实现未覆盖，按后续增强处理） |
...
校验 `param_key` 唯一、`value_type` 合法、平台专属和租户可编辑约束。具体类型值格式校验强度（当前实现未覆盖，按后续增强处理）。

### 4.6 异常处理

参数不存在、key 重复、租户不可编辑、平台专属、覆盖不存在、参数被业务引用（当前实现未覆盖，按后续增强处理）等返回业务异常。
...
管理接口校验菜单和操作权限；批量读取接口权限范围（当前实现未覆盖，按后续增强处理）。
...
- 异常处理：key 不存在时返回 null（当前实现未覆盖，按后续增强处理）。
