...
新增类型调用 `createDictType()`；新增字典项调用 `createDictItem()`。平台管理员维护默认定义；租户管理员是否可新增默认类型【待确认】。
...
|---|---|---|---|---|
| 按编码取字典项 | GET | `/api/dict-types/by-code/{code}/items` | 业务表单使用 | 登录【待确认】 |
...
校验类型编码、名称、scope、tenant_editable、is_platform_only、字典项 label/value、sort_order、enabled。同类型下 value 唯一约束未在模型中确认【待确认】。
...
`dict_type(tenant_id, code)` 唯一；`tenant_dict_item_override(tenant_id, dict_item_id)` 唯一。`dict_item` 未见 `(dict_type_id, value)` 唯一约束【待确认】。
...
管理接口使用菜单或操作权限；业务读取接口权限较宽，具体限制【待确认】。
