...
菜单管理由 `MenuView.vue` 实现权限菜单树、平台菜单维护和租户菜单覆盖。后端使用 `permissions.go`、`permission_service.go`、`permission.go` 管理 `permission` 表，并提供菜单包、租户覆盖、数据权限模式更新等接口。数据模型包括 `permission`、`tenant_menu_override`、`role_permission`、`saas_feature`。菜单是权限、套餐功能点、侧栏和按钮控制的共同来源。
...
| Application Service | 权限 Repository、菜单覆盖保存、功能点同步 | `internal/application/permission_service.go`, `internal/application/plan_service.go` |
...
前端主要通过 `fetchMenuBundles()`、`fetchPermissionMenuBundles()`、`fetchTenantMenuOverrides()` 获取菜单包和覆盖数据。菜单管理页是否还有独立分页查询 `/api/permissions` 调用【待确认】。

### 3.4 列表实现

菜单节点字段包括名称、路径、类型、权限码、排序、启用、平台专属、可套餐化、租户可见、租户可编辑、图标覆盖等。节点类型由 `permission.perm_type` 表示。

### 3.5 新增实现

平台新增菜单/按钮调用权限创建接口【前端封装待确认】。字段包括名称、路径、父级、权限类型、排序、启用、平台专属、可套餐化等。保存后触发功能点同步。
...
平台菜单状态字段为 `permission.enabled`；租户覆盖状态字段为 `tenant_menu_override.enabled`、`visible`。平台状态影响所有租户；租户覆盖只影响当前主体。
...
| 菜单包 | GET | `/api/permissions/menu-bundles` | 前端菜单和权限包 | 登录/菜单权限【待确认】 |
| 菜单覆盖 | GET | `/api/permissions/menu-overrides` | 查询租户覆盖 | 登录 |
| 保存菜单覆盖 | PUT | `/api/permissions/menu-overrides` | 保存覆盖 | `menu:create` 或 `menu:edit` |
| 数据权限模式 | PUT | `/api/permissions/menu-data-perm-mode/{permission_id}` | 更新菜单数据权限模式 | `menu:edit` 或 `permission:edit` |
...
`permission_service.go` 提供 `list_permission_page()`、`create_permission_for_viewer()`、`save_tenant_menu_overrides_for_viewer()`、`get_permission_for_viewer()`、`update_permission_for_viewer()`、`delete_permission_for_viewer()`。创建/编辑权限会调用 `plan_service.sync_feature_from_permission()` 同步套餐功能点。
...
权限不存在、路径重复【待确认】、父节点无效、平台专属误操作、租户覆盖不可编辑、权限被角色引用时删除等场景返回业务异常。
...
`tenant_menu_override(tenant_id, permission_id)` 唯一。`permission` 对 `tenant_id`、`parent_id`、`feature_code` 等建索引。路径唯一性约束未在模型中确认【待确认】。
...
前端侧栏为 `menu:create`、`menu:edit`、`menu:delete`；后端权限 Repository 也存在 `permission:create/edit/delete`。映射关系【待确认】。
...
| 保存菜单覆盖 | 是【待确认】 | 覆盖摘要 | `permission_service.go` |
...
- 触发入口：创建或编辑权限。
- 处理流程：若非平台专属且可套餐化，生成或更新 `saas_feature`。
- 涉及方法：`sync_feature_from_permission()`。
- 涉及数据：`permission`、`saas_feature`。
- 异常处理：功能编码冲突时更新已有记录。
- 注意事项：主体管理、套餐中心不得进入租户套餐池。
