...
| Application Service | 登录日志记录和查询范围【待确认】 | `internal/application/internal/application/identity`, `internal/application/log_service.go` |
...
`success` 为布尔或空；`account`、`ip`、`tenant_name_hint` 为字符串；日期范围为字符串日期。日期合法性、limit 上限【待确认】。
...
租户用户只能查询本主体登录日志；平台管理员和平台主体用户可使用主体筛选。登录失败无主体时如何归属【待确认】。
