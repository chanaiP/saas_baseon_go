...
本项目的业务对象默认不得物理删除。用户、角色、权限、租户、组织、业务单元、字典、参数、文件等可追溯业务数据必须使用 `deleted_at`、`status`、停用或归档表达删除结果，并由统一删除保护入口 `blockDeleteIfReferenced()` / `checkDeletionReferences()` 在删除前完成引用检查。
