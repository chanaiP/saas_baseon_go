...
- [ ] 设计并创建 `sys_app_manifest_load` 表，记录装载批次、来源、hash、版本、操作者、结果、错误摘要。
- [ ] 设计并创建 `sys_app_manifest_file` 表，记录上传或扫描到的 Manifest 文件、片段角色、hash 和摘要。
- [ ] 设计并创建应用资产表：`sys_app_entry`、`sys_app_api`、`sys_app_permission`、`sys_app_package_feature`、`sys_app_quota`。
- [ ] 补 GORM model、repository 和 schema/migration 文档。
- [ ] 实现 Manifest diff service，输出新增、更新、停用、冲突、阻断项。
- [ ] Manifest diff 支持多文件按 `app_code` 分组。
- [ ] Manifest diff 支持同一 `app_code` 多片段按 `fragment_role` 合并预览。
- [ ] Manifest diff 阻断同一 `app_code` 多个 `main`。
- [ ] Manifest diff 阻断菜单 path、权限码、API method + path、套餐功能码、配额码冲突。
- [ ] 实现 Manifest load service，确认后事务写入主档、客户端、菜单、权限、API、套餐功能和配额。
- [ ] Manifest load 写入装载记录和审计日志。
- [ ] Manifest load 支持失败回滚，禁止半成品数据。
- [ ] Manifest 删除资源时只生成停用、归档或移出套餐 diff，不物理删除。

## 3. P0 套餐与权限同步

- [ ] `package_features` 同步到 `saas_feature`。
- [ ] `quotas` 同步到 `saas_quota`。
- [ ] `menus` 和 `operations` 同步到 `permission`。
- [ ] `apis` 同步到 API 权限矩阵或应用 API 资产表。
- [ ] `include_in_package=false` 的能力不得进入套餐中心。
- [ ] 平台专属能力进入套餐中心时阻断装载。
- [ ] 菜单自动生成功能点逻辑降级为历史兼容，新增能力不再依赖该逻辑。

## 4. P0 防覆盖规则

- [ ] Manifest 重装载不得覆盖租户菜单覆盖。
- [ ] Manifest 重装载不得覆盖套餐列开关。
- [ ] Manifest 重装载不得覆盖角色授权。
- [ ] Manifest 重装载不得覆盖租户订阅。
- [ ] Manifest 重装载不得自动加回人工移出套餐的功能点。
- [ ] 人工覆盖、人工移出套餐、租户覆盖和角色授权必须有保护标记或可追溯来源。

## 5. P1 前端装载流程

- [ ] Manifest 上传后展示 diff 预览。
- [ ] Manifest 扫描后展示候选应用列表。
- [ ] 区分“新建导入”和“同步升级”。
- [ ] 多 Manifest 按 `app_code` 分组展示。
- [ ] 已存在 `app_code` 只能选择同步升级，不能新建导入。
- [ ] 冲突项高亮展示，并给出阻断原因。
- [ ] 确认装载后调用 load 接口。
- [ ] 装载成功后刷新应用列表、详情和装载记录。
...
- [ ] 详情页展示 Manifest 版本、hash、来源和最近同步时间。
- [ ] 详情页增加菜单/权限/API/套餐功能/配额页签。
- [ ] 详情页增加装载记录页签。
- [ ] 详情页展示同步状态和错误摘要。
- [ ] 详情页展示独立部署健康检查、API 地址、Webhook 地址和通讯模式。
