...
- [x] Manifest diff 支持多文件按 `app_code` 分组。
- [x] Manifest diff 支持同一 `app_code` 多片段按 `fragment_role` 合并预览。
- [x] Manifest diff 阻断同一 `app_code` 多个 `main`。
...
- [x] Manifest 删除资源时只生成停用、归档或移出套餐 diff，不物理删除。
...
- [x] 人工覆盖、人工移出套餐、租户覆盖和角色授权必须有保护标记或可追溯来源。
- [x] 历史菜单、权限、套餐功能和配额初始化进 Manifest 治理表，避免扫描时反复误判为新增。
- [x] `LEGACY_BASELINE` 历史基线资源不因当前 Manifest 未声明而进入停用 diff。

## 5. P1 前端装载流程

- [x] Manifest 上传后展示 diff 预览。
- [x] Manifest 扫描后展示候选应用列表。
- [x] 区分“新建导入”和“同步升级”。
- [x] 多 Manifest 按 `app_code` 分组展示。
...
- [x] 应用主档补 Webhook 地址。
- [x] 应用主档补通讯模式详情。
- [x] Manifest 校验独立部署应用必须声明通讯模式。
- [x] Manifest 校验独立部署应用不得缺少租户上下文、scope、签名或幂等策略说明。
- [x] 暂不发放真实凭证，只保留字段、校验口径和文档约束。
