...
- [ ] 文档：补充总览统计口径和用量明细数据源说明。
- [ ] 测试：补充总览聚合 service 单测。
- [ ] Review：检查总览 SQL 聚合、空数据兜底、数值格式和性能风险。

## 5. 菜单二：供应商

- [ ] 后端：供应商整体导入接口支持 providers/accounts/apis 一次性导入。
- [ ] 后端：供应商整体导入做 provider/account/api 引用校验和事务回滚。
- [ ] 后端：删除供应商时处理账号/API 级联逻辑，并在引用模型/用量时阻断。
- [ ] 后端：删除账号时逻辑删除账号下 API。
- [ ] 供应商页改为三级管理：供应商列表、接入账号、API 配置。
- [ ] 供应商页提供供应商/账号/API 三类表单抽屉，不依赖通用 JSON 编辑。
- [ ] 供应商页提供整体导入入口和导入结果反馈。
- [ ] 文档：补充供应商整体导入 payload 示例。
- [ ] 测试：补充供应商导入、级联删除和引用阻断 service 单测。
- [ ] Review：检查供应商账号密钥字段不回显、事务边界、软删除一致性。

## 6. 菜单三：模型目录

- [ ] 后端：模型导入接口支持模型、价格策略、分档价格一次性导入。
- [ ] 后端：模型导入做 provider/model/policy/tier 引用校验和事务回滚。
- [ ] 后端：删除模型时阻断已被路由模型池或用量明细引用的模型。
- [ ] 模型目录页改为供应商侧栏 + 模型列表。
- [ ] 模型目录页提供模型详情抽屉。
- [ ] 模型目录页提供价格策略抽屉和分档价格编辑。
- [ ] 模型目录页提供模型/价格整体导入入口。
- [ ] 文档：补充模型、价格策略、分档价格导入 payload 示例。
- [ ] 测试：补充模型导入和价格策略分档单测。
- [ ] Review：检查价格字段精度、分档匹配字段、供应商筛选和空状态。

## 7. 菜单四：AI 场景

- [ ] 后端：AI 场景导入接口支持批量 upsert。
- [ ] 后端：AI 场景导入校验 `app_code + ai_scenario_code` 唯一、能力字典存在、默认基础路由存在。
- [ ] 后端：删除场景时阻断被租户策略或用量明细引用。
- [ ] AI 场景页按应用分组展示。
- [ ] AI 场景页能力编码从能力字典选择。
- [ ] AI 场景页默认基础路由从路由列表选择。
- [ ] AI 场景页展示租户覆盖策略数。
- [ ] 文档：补充 AI 场景注册和导入 payload 示例。
- [ ] 测试：补充 AI 场景批量 upsert 和引用阻断单测。
- [ ] Review：检查场景注册强约束、能力字典选择、默认路由选择一致性。
...
- [ ] 应用中心：每次菜单/权限/API/套餐变更后执行 manifest scan/load 或等价装载验证。
