...
## 15.1. Gateway 策略执行端点闭环

- [x] 明确多账号、多 API 场景下，租户策略只选择基础路由，账号/API 由基础路由执行节点解析。
- [x] 基础路由模型节点增加可选 `provider_account_id` 和 `provider_api_id`，支持平台显式绑定执行账号/API。
- [x] Gateway 调用从“模型 -> 供应商默认账号/API”升级为“模型节点 -> 绑定端点或健康优先自动端点”。
- [x] 执行端点选择校验模型供应商、账号归属、API 归属、API 能力、账号/API 启用状态和连通性错误状态。
- [x] 基础路由导入支持账号/API 绑定字段，并校验绑定端点归属关系。
- [x] 基础路由页面展示模型节点对应的执行端点，自动选择和显式绑定都可见。
- [x] 测试：补充显式绑定 API 命中、绑定 API 异常时跳过并回退的 Gateway 单测。
- [x] Review：确认用量记录仍写入最终 `provider_account_id` 和 `provider_api_id`，保证后续审计能追踪真实落点。
...
- [x] API 连通性手动检测支持按当前供应商/账号列表范围执行，避免在过滤视图中误触发全量检测。
- [x] 供应商页接入账号和 API 列表改为服务端分页，并让连通性检测只检测当前 API 表格页。
...
- [x] 初始化真实供应商目录种子数据，覆盖 13 个主流供应商、真实 API endpoint、生产账号别名、模型能力、价格策略和默认基础路由。
- [x] 清理旧 demo 供应商账号、API 和演示路由残留，确保页面展示为标准 `prod-main` 生产接入目录。
- [x] 供应商按原型恢复供应商侧栏、接入账号、API 配置三段结构。
- [x] 供应商页调整为左右栏工作台：左侧供应商卡片维护，右上账号统计与账号表，右下 API 统计与 API 表；支持默认全量、按供应商过滤、按账号过滤 API。
- [x] 供应商账号表移除“筛选 API”按钮，改为账号行 hover/点击选中并联动下方 API 列表。
- [x] 供应商/账号/API 编辑弹窗挂载到页面 body，避免被供应商卡片滚动容器裁剪。
- [x] 接入账号补充注册方式、注册账号、维护人和联系方式，支持邮箱、手机、第三方登录等真实维护场景。
