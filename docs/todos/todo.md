...
- [x] 基础路由页提供模型池编辑器，支持 role、priority、weight、retry、timeout。
- [x] 基础路由页策略枚举覆盖 fixed/fallback/priority/load_balance/cost_first/quality_first/latency_first/quota_aware/tenant_custom/capability_match。
- [x] 文档：补充基础路由和模型池导入 payload 示例。
- [x] 测试：补充基础路由批量 upsert、模型池校验、引用阻断单测。
- [x] Review：检查模型池编辑交互、策略枚举、删除边界。

## 9. 菜单六：策略中心

- [x] 后端：租户策略导入接口支持 policies/quota-rules/rate-limit-rules 批量 upsert。
- [x] 后端：租户策略导入校验 AI 场景、默认路由、覆盖路由和规则维度。
- [x] 后端：删除策略时逻辑删除配额规则和限流规则。
- [x] 策略中心页提供策略抽屉。
- [x] 策略中心页支持多条配额规则编辑。
- [x] 策略中心页支持多条限流规则编辑。
- [x] 策略中心页展示策略优先级说明。
- [x] 文档：补充租户策略、配额规则、限流规则导入 payload 示例。
- [x] 测试：补充租户策略批量 upsert、规则编辑和删除级联单测。
- [x] Review：检查策略优先级、规则维度、超限动作和多规则表单。
...
- [x] 运行前端生产构建。
- [x] 运行 `git diff --check`。
- [x] 复核移动端和桌面端布局不溢出、不重叠。
- [x] Review 前端交互：确认没有用通用 JSON 编辑替代生产表单。
...
- [x] 执行 `git diff --check`。
- [x] Review 后端资源边界：是否存在未校验引用、错误软删除、事务缺失。
- [x] Review 文档：菜单、接口、部署说明是否与代码一致。
- [x] 修复 review 发现的问题并回归测试。

## 15. 提交

- [x] 每完成一组可验证功能后更新本 TODO 勾选。
- [x] 每完成一组可验证功能后提交独立 commit。
- [x] 最终汇总完成项、测试结果、review 结论和残余风险。

## 16. 原型结构还原与菜单页拆分

- [x] 使用 `/frontend-design` 技能复核原型约束：结构跟随 demo，视觉仅做暗色主题适配。
- [x] 将 AI 能力中心从单个 `AiCapabilityCenterView` 拆为 7 个路由组件：总览、供应商、模型目录、AI 场景、基础路由、策略中心、系统设置。
- [x] 更新前端路由 name 与后台菜单组件映射，确保每个 demo 左侧项都是平台菜单页，不是页内 tab。
- [x] 总览按原型恢复 hero、指标卡、趋势、成本结构、租户排行榜、健康检查、核心路由。
- [x] 总览调用趋势按原型恢复为折线/面积趋势图，移除横向进度条列表表现。
- [x] 成本结构和相关模型类型展示统一翻译为中文，避免直接暴露 `text/image/embedding` 枚举。
- [x] 统一修复 AI 能力中心 Element Plus 表格底纹，避免表体出现黑色断层。
- [x] 移除 7 个 AI 能力中心菜单页内自带刷新按钮，刷新统一交给框架能力。
- [x] AI 能力中心样式支持框架深色/浅色主题切换，卡片、表格、输入和图表颜色不再写死深色。
- [x] 统一调整 AI 能力中心字体字号、行高和指标卡密度，覆盖总览、供应商、模型目录、AI 场景、基础路由、策略中心、系统设置页面。
- [x] 保留 AI 能力中心卡片原有层次阴影，仅移除卡片间隙容器底纹，保持深色/浅色模式下卡片边界干净。
- [x] 总览页 AI Gateway 状态胶囊改为基于后端健康检查动态展示，并修复胶囊与标题文字粘连问题。
- [x] AI 能力中心人民币金额统一使用千分位格式展示。
- [x] 平台健康检查加入 AI 场景绑定可用性、API 连通性检测和 10 分钟定期检测闭环。
- [x] API 连通性检测改为真实供应商接口调用，覆盖 OpenAI/OpenAI-Compatible、Azure OpenAI、DashScope、火山方舟、DeepSeek、智谱、Moonshot、百川、MiniMax、腾讯混元、百度千帆、Anthropic 等主流接入。
- [x] 供应商按原型恢复供应商侧栏、接入账号、API 配置三段结构。
- [x] 模型目录按原型恢复供应商侧栏、模型列表、价格策略、分档价格。
- [x] AI 场景按原型恢复应用侧栏、注册表、调用标准。
- [x] 基础路由按原型恢复执行流程、路由指标、模型池表、命中逻辑。
- [x] 策略中心按原型恢复策略主表、配额/限流规则摘要、优先级说明。
- [x] 系统设置按原型恢复网关配置、安全归属、能力字典、环境变量示例。
- [x] 执行前端构建和代码 review，修复类型、样式、路由问题。
- [x] 浏览器访问 7 个菜单页，确认中文菜单和操作入口可见。
- [x] 提交原型结构还原变更。
