...
JOIN saas_plan p ON p.plan_code = d.plan_code AND p.deleted_at IS NULL
JOIN saas_quota q ON q.quota_code = d.quota_code AND q.status = 1
ON CONFLICT (plan_id, quota_id)
DO UPDATE SET quota_value = EXCLUDED.quota_value, updated_at = now();
