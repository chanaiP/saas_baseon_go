...
func (s *AppService) CreateApp(ctx context.Context, viewerID uint64, req dto.AppCreateRequest) (dto.AppResponse, error) {
	...
	row := models.SysApp{
		...
		HealthCheckURL:  cleanOptionalString(req.HealthCheckURL),
		APIBaseURL:      cleanOptionalString(req.APIBaseURL),
		WebhookURL:      cleanOptionalString(req.WebhookURL),
		IsBuiltin:       false,
...
func (s *AppService) UpdateApp(ctx context.Context, viewerID uint64, id uint64, req dto.AppUpdateRequest) (dto.AppResponse, error) {
	...
	updates := map[string]interface{}{
		...
		"health_check_url":    cleanOptionalString(req.HealthCheckURL),
		"api_base_url":        cleanOptionalString(req.APIBaseURL),
		"webhook_url":         cleanOptionalString(req.WebhookURL),
...
func appToResponse(row models.SysApp, clients []models.SysAppClient) dto.AppResponse {
	...
	return dto.AppResponse{
		...
		LastManifestSync: row.LastManifestSync,
...
func (s *AppService) appAssets(ctx context.Context, appCode string) (dto.AppAssetsResponse, error) {
	...
	for _, row := range entries {
		result.Entries = append(result.Entries, dto.AppEntryResponse{
			...
			TenantScope:      defaultString(row.TenantScope, "enterprise_only"),
